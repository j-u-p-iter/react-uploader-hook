...
import { DropZone, MAX_SIZE_ERROR, FILE_TYPE_ERROR } from '.';
...
describe('uploaderHook', () => {
  ...
  beforeAll(() => {
    ...
    checkOnAttach = (firstExpectedArgument, secondExpectedArgument, callIndex = 0) => {
    ...
    renderComponent = (props = {}) => {
      return render(
        <DropZone {...props}>
          {({ getRootProps, getInputProps, acceptedFilesData, rejectedFilesData, deleteFile, deleteAllFiles }) => {
            return (
              <>
                <div {...getRootProps()} data-testid={testIds.DND_AREA}>
                  <input {...getInputProps()} data-testid={testIds.INPUT} type='file' />
                </div>
  ...
  describe('Input', () => {
    ...
    describe('size validation', () => {
      it('provides a correct error when a size is above {maxSize}', async () => {
        ...
        const { getByTestId, queryByTestId } = renderComponent({ maxSize: 1000, onAttach });

        fireEvent.change(getByTestId(testIds.INPUT), { target: { files: [file] } });
      ...
      it('does not provide an error when a size is lower or equal {maxSize}', async () => {
        ...
        const { queryByTestId, getByTestId } = renderComponent({ maxSize: 1000, onAttach });
      ...
      it('does not accept rejected files in case of uploading of multiple files', () => {
        ...
        const { getByTestId } = renderComponent({ maxSize: 5000, multiple: true, onAttach });

        fireEvent.change(getByTestId(testIds.INPUT), { target: { files: [file1, file2] } });
    ...
    describe('attaching files', () => {
      ...
      it('provides information about all attached files in case of uploading of multiple files', () => {
        ...
        const { queryAllByTestId, getByTestId } = renderComponent({ multiple: true, onAttach });

        fireEvent.change(getByTestId(testIds.INPUT), { target: { files: [file1, file2] } });

        expect(queryAllByTestId('acceptedFileName')[0]).toHaveTextContent(file1.name);
        expect(queryAllByTestId('filePreview')[0]).toHaveAttribute('src');

        expect(queryAllByTestId('acceptedFileName')[1]).toHaveTextContent(file2.name);
        expect(queryAllByTestId('filePreview')[1]).toHaveAttribute('src');

        checkOnAttach(
          [{ file: file1, preview: 'url-to-file' }, { file: file2, preview: 'url-to-file' }], 
          []
        );
      });

      it('attaches new information to the previous one instead of rewriting it', () => {
        const file1 = createFile({ name: 'super.pdf', size: 900 });
        const file2 = createFile({ name: 'super.png', size: 5200 });
        ...
        fireEvent.change(getByTestId(testIds.INPUT), { target: { files: [file3, file4] } });

        expect(queryAllByTestId('acceptedFileName').length).toBe(4)
        expect(queryAllByTestId('filePreview').length).toBe(4)
        expect(queryAllByTestId('acceptedFileName')[2]).toHaveTextContent(file3.name);
        expect(queryAllByTestId('filePreview')[2]).toHaveAttribute('src');
        expect(queryAllByTestId('acceptedFileName')[3]).toHaveTextContent(file4.name);
  ...
  describe('DnD area', () => {
    describe('file type validation', () => {
      it('provides a correct error in case of incorrect file type', async () => {
        const file = createFile({ type: 'image/jpeg', name: 'super.jpeg', size: 1200 });

        const { getByTestId, queryByTestId } = renderComponent({ accept: 'image/png', onAttach });

        fireDropEvent({ element: getByTestId(testIds.DND_AREA), files: [file] });
      ...
      it('provides a correct error in case of invalid extension', async () => {
        const file = createFile({ type: '.jpeg', name: 'super.jpeg', size: 1200 });

        const { getByTestId, queryByTestId } = renderComponent({ accept: '.png', onAttach });
      ...
      it('does not provide an error if file type is valid', () => {
        ...
        const { queryByTestId, getByTestId } = renderComponent({ accept: 'image/*', onAttach });
      ...
      it('does not provide an error if file extension is valid', () => {
        ...
        const { queryByTestId, getByTestId } = renderComponent({ accept: '.png', onAttach });
      ...
      it('does not accept rejected files in case of uploading of multiple files', () => {
        const file1 = createFile({ type: '.pdf', name: 'super.pdf', size: 900 });
        const file2 = createFile({ type: 'image/png', name: 'super.png', size: 5200 });

        const { getByTestId } = renderComponent({ accept: '.pdf', multiple: true, onAttach });
    ...
    describe('size validation', () => {
      ...
      it('does not accept rejected files in case of uploading of multiple files', () => {
        ...
        fireDropEvent({ element: getByTestId(testIds.DND_AREA), files: [file1, file2] });
    ...
    describe('attaching files', () => {
      ...
      it('attaches new information to the previous one instead of rewriting it', () => {
        ...
        fireDropEvent({ element: getByTestId(testIds.DND_AREA), files: [file3, file4] });
  ...
  describe('deleting files', () => {
    ...
    beforeEach(() => {
      ...
      fireDropEvent({ element: renderResult.getByTestId(testIds.DND_AREA), files: [file1, file2] });
    })
    ...
    it('deletes one file properly', () => {
      ...
      expect(queryAllByTestId('acceptedFileName').length).toBe(1);
      expect(queryAllByTestId('acceptedFileName')[0]).toHaveTextContent(file2.name);
    });
