...
import { MAX_SIZE_ERROR, FILE_TYPE_ERROR } from './constants';


export const isDragEvent = (event: ChangeEvent<HTMLInputElement> | DragEvent): event is DragEvent => 
  event.hasOwnProperty('dataTransfer');

type GetFilesFromEvent = (event: ChangeEvent<HTMLInputElement> | DragEvent) => FileList;
...
const checkFileType: CheckFileType = (file, accept) => {
  if (!accept || isAccepted(file, accept)) { return; }
  
  return FILE_TYPE_ERROR;
}

type ValidateFile = (params: { file: File; maxSize?: number; accept?: string; }) => string[];
